                    ...
                    <form action="{{ route('admin.tickets.update', $ticket) }}" method="POST" enctype="multipart/form-data">
                        ...
                        <div class="row g-3">
                            <div class="col-md-6">
                                ...
                                <input type="text" class="form-control" value="{{ $ticket->numero_reporte }}" disabled>
                            </div>
                            ...
                            <div class="col-md-6">
                                ...
                                <select name="nivel_urgencia" class="form-select">
                                    @foreach (['baja', 'media', 'alta', 'critica'] as $nivel)
                                        <option value="{{ $nivel }}" {{ $ticket->nivel_urgencia === $nivel ? 'selected' : '' }}>
                                            {{ ucfirst($nivel) }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div class="col-12">
                                ...
                                <textarea name="descripcion_detallada" class="form-control" rows="3">{{ $ticket->descripcion_detallada }}</textarea>
                            </div>
                            ...
                            <div class="col-md-6">
                                ...
                                <input
                                    ...
                                    class="form-control"
                                    value="{{ $ticket->fecha_resolucion?->format('Y-m-d\TH:i') }}"
                                >
                            </div>

                            <div class="col-12">
                                <label class="form-label fw-semibold">Comentarios Técnico</label>
                                <textarea name="comentarios_tecnico" class="form-control" rows="3">{{ $ticket->comentarios_tecnico }}</textarea>
                            </div>
                        ...
                        <div class="mt-4 d-flex gap-2">
                            ...
                            <a href="{{ route('admin.tickets.show', $ticket) }}" class="btn btn-secondary">Cancelar</a>
                        </div>
                    </form>
                </div>
            </div>
